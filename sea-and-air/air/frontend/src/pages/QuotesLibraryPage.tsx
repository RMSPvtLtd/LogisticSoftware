  ...
  const visible = useMemo(() => {
    ...
    return (records.data?.quotes ?? [])
      ...
      .filter((quote) => {
        ...
        return [quote.id, quote.inquiry_id, quote.carrier, quote.origin, quote.destination, customer?.name, customer?.company_name]
  ...
  return <>
    <div className="hidden max-h-[min(62vh,46rem)] overflow-auto rounded-xl border border-border lg:block">
      <table className="w-full text-sm">
        ...
        <tbody>{quotes.map((quote) => { const { inquiry, customer } = context(quote); return <tr key={quote.id} className="border-t border-border transition-colors hover:bg-muted/50"><td className="p-3"><Link to={`/quotes/${quote.id}`} className="font-medium tabular-nums hover:underline">Q-{quote.root_quote_id ?? quote.id} · Rev {quote.revision_number}</Link><p className="text-xs text-muted-foreground">Inquiry #{quote.inquiry_id}</p></td><td className="p-3">{customer?.name ?? "—"}</td><td className="p-3 whitespace-nowrap">{inquiry ? `${inquiry.origin} → ${inquiry.destination}` : `${quote.origin} → ${quote.destination}`}</td><td className="p-3">{quote.carrier ?? "—"}</td><td className="p-3">{quote.is_manual ? "Manual" : "Rate card"}</td><td className="p-3"><Badge variant="outline">{STATUS_LABEL[quote.status]}</Badge></td><td className="p-3 text-right font-medium tabular-nums">{formatMoney(quote.total, quote.currency)}</td><td className="p-3 whitespace-nowrap">{formatDate(quote.valid_until)}</td><td className="p-3 whitespace-nowrap text-muted-foreground">{formatDateTime(quote.updated_at)}</td><td className="p-3 text-right"><Button asChild variant="ghost" size="sm"><Link to={`/quotes/${quote.id}`}>Open</Link></Button></td></tr>})}</tbody>
      </table>
    </div>
    <ul className="space-y-2 lg:hidden" aria-label="Quote records">{quotes.map((quote) => { const { inquiry, customer } = context(quote); return <li key={quote.id} className="rounded-xl border border-border p-4"><div className="flex items-start justify-between gap-3"><div><Link to={`/quotes/${quote.id}`} className="font-medium tabular-nums hover:underline">Q-{quote.root_quote_id ?? quote.id} · Rev {quote.revision_number}</Link><p className="mt-0.5 text-sm text-muted-foreground">{customer?.name ?? `Inquiry #${quote.inquiry_id}`}</p></div><Badge variant="outline">{STATUS_LABEL[quote.status]}</Badge></div><p className="mt-3 text-sm">{inquiry ? `${inquiry.origin} → ${inquiry.destination}` : `${quote.origin} → ${quote.destination}`}</p><div className="mt-3 flex items-end justify-between gap-3"><div className="text-xs text-muted-foreground"><p>{quote.carrier ?? "Unspecified carrier"} · {quote.is_manual ? "Manual" : "Rate card"}</p><p>Valid {formatDate(quote.valid_until)}</p></div><p className="font-heading font-semibold tabular-nums">{formatMoney(quote.total, quote.currency)}</p></div></li>})}</ul>
  </>
}
